                ...
                <div class="cannext_wrap" style="width: 320px" align="center">
                	<c:choose>
	                	<c:when test="${login.ID == map.WRITER }">
		                    ...
		                    <div class="btn cancel" onclick="location.href='/highExam'" style="float: right">목록으로</div>
	                	</c:when>
	                	<c:otherwise>
	                		<div class="btn cancel" onclick="location.href='/highExam'">목록으로</div>
	                	</c:otherwise>
                	</c:choose>
                </div>
            </div>
		...
		function modify(){
    	 ...
    	 // 삭제 완료
      	function commit(){
      		$.ajax({
      			type : "post",
      			url : "/highExam/delete/${map.AUTO}",
      			async : false,
      			success : function(txt){
      				if(txt){
      					alert("삭제되었습니다.");
      					location.href="/highExam";
      				} else {
      					alert("삭제에 실패하였습니다.\n잠시후 다시 시도해주세요.");
      				}
      			}
