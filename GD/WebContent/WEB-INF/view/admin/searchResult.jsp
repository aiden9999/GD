...
<script language="javascript">
	...
	function setParent(){
		...
		var returnUrl = "http://127.0.0.1/admin/postCode";
	...
	function init(){
		...
		if("P0001" =="E0005"){
			alert("검색결과가 너무 많습니다. 도로명주소 또는 지번을 입력하세요");
		}
	}
		...
		<div class="pop-address-search" style="width: 100%;" >
			...
			<div style="height: 360px;" id="serarchContentBox">
				<fieldset>
					...
					<span class="wrap">
						<input type="text" class="popSearchInput" style="font-size: 13px; font-weight: bold; ime-mode:active;" title="검색어를 입력하세요" name="keyword" id="keyword" placeholder="도로명주소, 건물명 또는 지번입력" value="대청로" onkeydown="enterSearchUrl();" tabindex="1"/>
				...
				<!-- 검색결과 -->
					...
					<div class="result" id ="resultList" style="margin-top:7px;">
						<p class="text-guide">도로명주소 검색 결과 <strong>(1건)</strong></p>
						<table class="data-col" style="margin-top:3px;">
				            ...
				            <tbody>
				            	<!-- loop -->
				            	<!-- 20160912 추가,  1 : 도로명만 나옴, 2 : 도로명+지번, 3 : 도로명+상세건물명, 4 : 기본 출력 -->
				            	<c:forEach var="i" begin="1" end="${roadFullAddr.size()-1 }">
				            		<tr>
					                	<td class="subj" style="text-align:center;">${i }</td> 
						            	
										<td class="subj" colspan="2" id="roadAddrTd${i }" style="text-align:left; padding-left: 5px; line-height: 0.5em;" >
											<a href="javascript:setMaping('${i }')" > 	 
												<div tabindex="6">
													<div id="roadAddrDiv${i }" ><b>${resultList.get(i).roadFullAddr }</b></div>
													<span style="font-size:11px;">[지번] <span id="jibunAddrDiv${i }">${resultList.get(i).jibunAddr }</span></span>
												</div>
											</a>
											<div id="roadAddrPart1Div${i }" style="display:none;">${resultList.get(i).roadAddrPart1 }</div>
											<div id="roadAddrPart2Div${i }" style="display:none;"> ${resultList.get(i).roadAddrPart1 }</div>
											<div id="engAddrDiv${i }" style="display:none;">${resultList.get(i).engAddr }</div>
										</td> 
										
										<td class="subj" style="text-align:center" id="zipNoTd1"> 
											<div id="zipNoDiv1">${resultList.get(i).zipNo }</div>
										</td>
						                <input type="hidden" id="admCdHid${i }" value="${resultList.get(i).admCd }"/>    
										<input type="hidden" id="rnMgtSnHid${i }" value="${resultList.get(i).rnMgtSn }"/>    
										<input type="hidden" id="bdMgtSnHid${i }" value="${resultList.get(i).bdMgtSn }"/> 
					                </tr>
				            	</c:forEach>
				                <!-- //loop -->
				            </tbody>
							...
							<div class="paginate">
					            <strong>1</strong><a href="?currentPage=2&countPerPage=5&searchType=TOTAL&searchKeyword=%EB%8C%80%EC%B2%AD%EB%A1%9C&lang=&sortType=acc">2</a><a href="?currentPage=3&countPerPage=5&searchType=TOTAL&searchKeyword=%EB%8C%80%EC%B2%AD%EB%A1%9C&lang=&sortType=acc">3</a><a href="?currentPage=4&countPerPage=5&searchType=TOTAL&searchKeyword=%EB%8C%80%EC%B2%AD%EB%A1%9C&lang=&sortType=acc">4</a><a href="?currentPage=5&countPerPage=5&searchType=TOTAL&searchKeyword=%EB%8C%80%EC%B2%AD%EB%A1%9C&lang=&sortType=acc">5</a><a href="?currentPage=6&countPerPage=5&searchType=TOTAL&searchKeyword=%EB%8C%80%EC%B2%AD%EB%A1%9C&lang=&sortType=acc">6</a><a href="?currentPage=7&countPerPage=5&searchType=TOTAL&searchKeyword=%EB%8C%80%EC%B2%AD%EB%A1%9C&lang=&sortType=acc">7</a><a href="?currentPage=8&countPerPage=5&searchType=TOTAL&searchKeyword=%EB%8C%80%EC%B2%AD%EB%A1%9C&lang=&sortType=acc">8</a><a href="?currentPage=9&countPerPage=5&searchType=TOTAL&searchKeyword=%EB%8C%80%EC%B2%AD%EB%A1%9C&lang=&sortType=acc">9</a><a href="?currentPage=10&countPerPage=5&searchType=TOTAL&searchKeyword=%EB%8C%80%EC%B2%AD%EB%A1%9C&lang=&sortType=acc">10</a><a class='skip next' href="?currentPage=11&countPerPage=5&searchType=TOTAL&searchKeyword=%EB%8C%80%EC%B2%AD%EB%A1%9C&lang=&sortType=acc">다음으로</a>
					        </div>
