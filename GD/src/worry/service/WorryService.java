...
public class WorryService {
	...
	public void replyUp(int num){
		SqlSession ss = fac.openSession();
		ss.update("worry.replyUp", num);
		ss.commit();
		ss.close();
	}
	
	// °Ë»ö Ajax °í¹Î ¸®½ºÆ®
	public List<HashMap> worryAjax(String search, String word, int page){
		SqlSession ss = fac.openSession();
		HashMap map = new HashMap();
		map.put("word", "%"+word+"%");
		map.put("page", (page-1)*10);
		List<HashMap> list = ss.selectList("worry.worryAjax"+search, map);
		for(int i=0; i<list.size(); i++){
			HashMap m = list.get(i);
			String id = (String)m.get("WRITER");
			String id1 = id.substring(0, id.length()/2);
	...
	public int searchPage(String search, String word){
		SqlSession ss = fac.openSession();
		int n = ss.selectOne("worry.searchPage"+search, "%"+word+"%");
		ss.close();
		return n;
	}
}
